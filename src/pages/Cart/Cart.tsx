...
const Cart = () => {
  const { cart, setCart, setOrderList } = useContext(
    ProductStoreContext
  ...
  const handleConfirmOrder = () => {
    // Move products from cart to order list
    setOrderList((prevOrderList) => [...prevOrderList, ...cart]);
    // Clear the cart
    setCart([]);
  };
      ...
      <div className="grid grid-cols-1 w-1/2 mx-auto">
        {cart && cart.length > 0 ? (
          <>
            ...
            <button
              className="btn btn-outline-primary border-orange-600 bg-orange-600 hover:bg-transparent hover:text-orange-600 text-white font-body px-8"
